...
module.exports = function (controller) {
	...
	welcomeConvo.addQuestion(
		...
		async (response, convo, bot) => {
			if (!/^[1-9]\d{9}$/.test(response)) {
				...
				await bot.say({ type: 'typing' });
				await delay(4000);
				await convo.gotoThread('askCode');
			}
		},
		'varNumber',
		'askNumber'
	...
	welcomeConvo.addAction('askCode');

	welcomeConvo.addQuestion(
		'Scratch and enter the unique code (Presenr in the necktag)',
		async (response, convo, bot) => {
			await bot.say({ type: 'typing' });
			await delay(4000);
			await bot.say('Unique code validated');
			...
			await bot.say({ type: 'typing' });
			await delay(4000);
			await bot.say('Now answer this simple question correctly & stand a chance to win passes to Zomaland,Bangalore 4th-5th February');
	...
	welcomeConvo.after(async (results, bot) => {
		if (results.varAge !== 'No') {
			...
			var config = {
				method: 'post',
				url: 'https://backend.unomok.com/items/black_white_bengaluru',
				headers: {
